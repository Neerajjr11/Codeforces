...
void solve()
{
	int n; cin >> n;
	int k; cin >> k;
	// If k lies in the left half i.e in the odd half
	//  and multiply by 2*k -1 to get corresponding k odd element
	if (k <= (n + 1) / 2)
		cout << 2 * k - 1 << endl;
	else
		// If k lies in the right half i.e even half
		//  we subtract it from n+1/2 so tht to get the proceeding even element corresponding to k
		//  and multiply by 2*k to get the even element
	{	
		k -= ((n + 1) / 2);
		cout << 2 * k << endl;
	}
