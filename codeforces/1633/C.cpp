...
void solve()
{
    //    H and D is for hero
    //    h and d is monster
    // time taken for monster to die : ceil(h/D) = h + D - 1 / D
    // time taken for hero to die : ceil(H/d) = H + d - 1 / d
    //    for hero to win ceil(H/d) >= ceil(h/D)
    int H, D, h, d;
    cin >> H >> D >> h >> d;
    int k, w, a;
    cin >> k >> w >> a;
    rep(i, 0, k + 1)
    {
        int health = H + (i * a);
        int dmg = D + ((k - i) * w);
        int time_hero = ((health + d - 1) / d);
        int time_monster = ((h + dmg - 1) / dmg);
        if (time_hero >= time_monster)
        {
            print("YES");
            return;
        }
