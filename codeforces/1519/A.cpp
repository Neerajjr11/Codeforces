...
void solve()
{
    int r, b, d;
    read(r, b, d);
    // r≤b  (otherwise, we can swap them)
    //  Note that you can't use more than r packets (at least one red bean in each packet), so b can't exceed r⋅(d+1) (at most d+1 blue beans in each packet).
    if (b < r)
        swap(r, b);
    if (b <= (r * (d + 1)))
        print("YES");
