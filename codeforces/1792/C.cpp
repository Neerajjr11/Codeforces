...
{
    int n;
    int maxi, start, end, cur_start, cur_end, mid, res, ans;
    read(n);
    vi p(n);
    vi b(n + 1);
    rep(i, 0, n)
    ...
    maxi = start = end = cur_start = cur_end = 1;
    mid = (n + 1) / 2;
    int mini_mid = min(mid, cur_end);
    int maxi_mid = max(mid, cur_end);
    int c1 = mid - mini_mid + cur_start - 1;
    int c2 = (n - maxi_mid);
    ans = max(c1, c2);
    res = min(mid, ans);
    rep(i, 2, n + 1)
    {
        ...
        mini_mid = min(mid, cur_end);
        maxi_mid = max(mid, cur_end);
        c1 = mid - mini_mid + cur_start - 1;
        c2 = (n - maxi_mid);
        ans = max(c1, c2);
        amin(res, ans);
    }
