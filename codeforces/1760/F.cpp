...
#define summ(x) accumulate(x.begin(), x.begin() + min(z, (n)), 0LL);
#define summ1(x) accumulate(x.begin(), x.begin() + min((d % z), (n)), 0LL);
...
int helper(int z, int n, int c, int d, vi &q)
{
    int sumi = summ(q);
    int posi = (d / z) * sumi + summ1(q);
    return (posi >= c);
}
void solve()
{
