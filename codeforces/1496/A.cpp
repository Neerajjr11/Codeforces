...
void solve()
{
    // here we first need to check if length of string is greater than or equal to 2 * k + 1
    // second is the first k characters and the last k characters of the string should form a palindrome if concatenated
    // because the rest part in the middle can be assigned to sk+1
    int n, k;
    cin >> n >> k;
    string s;
    read(s);
    ...
    string new_s = s.substr(0, k) + s.substr(n - k, k);
    // print(new_s);
    string rev = new_s;
