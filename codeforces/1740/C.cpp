...
void solve()
{
	...
	// int minw = vect[0];
	// int sminw = vect[1];
	// cout << max(((2 * maxw) - smaxw - minw) , ( abs(minw - sminw) + abs(minw - maxw))) << endl;

	rep(i, 0, n - 1) {
		res = max(res, (abs(vect[i + 1] - vect[i]) + abs(vect[i] - vect[n - 1])));

	}
	rep(i, 1, n) {
		res = max(res, (abs(vect[i] - vect[i - 1]) + vect[i] - vect[0]));
	}
