...
void solve()
{
    //  the idea is to check whether we can sort the string after removing non-adjacent characters
    // since sorted strings follow 1's after 0's
    // we have to simple check that after the first one there shouldn't be any adjacent zeros
    // "00" can't be after "11"
    string s;
    read(s);
    int ind = -1;
    int ind0 = 0;
