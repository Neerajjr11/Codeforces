...
template <typename T1, typename T2>
void __print(const pair<T1, T2> &x)
{
    cerr << '{';
    __print(x.ff);
    cerr << ',';
...
void solve()
{
    ...
    unsigned int acc = 0;
    unsigned int numerator = n * (n - 1);
    
    rep(i, 1, s + 1)
    {
        acc += ((numerator%modQ) * inverse(2*i*i%modQ))%modQ ;
    }
    cout << ((acc % modQ)) << endl;
}
int32_t main()
